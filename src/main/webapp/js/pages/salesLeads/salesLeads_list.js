function addSalesLeads() {
    var url = ctx + "/inner/salesLeads/forwardEdit";
    layer.open({
        type: 2,
        title: '新增销售线索',
        shadeClose: true,
...
function editSalesLeads(id) {
    var url = ctx + "/inner/salesLeads/forwardEdit?id=" + id;
    layer.open({
        type: 2,
        title: '编辑销售线索',
        shadeClose: true,
...
function viewDetail(id, salesLeadsName) {
    var url = ctx + "/inner/salesLeads/detail?id=" + id;
    var title = "销售线索【" + salesLeadsName + "】";
    layer.open({
        type: 2,
        title: title,
        shadeClose: true,
...
function batchDelete() {
    ...
    if (isNotBlank(deleteIdArr)) {
        layer.confirm("确定要批量删除销售线索相关信息吗？", {
            closeBtn: false,
            skin: 'layui-layer-molv'
        }, function () {
            var url = ctx + "/inner/salesLeads/batchDeleteById?ids=" + deleteIdArr;
            $.ajax({
                type: "get",
                url: url,
                success: function (result) {
                    if (result == 2) {
                        layer.alert("批量删除销售线索成功", {
                            closeBtn: false,
                            skin: 'layui-layer-molv'
                        }, function () {
                            refreshTable();
...
function batchTransfer() {
    ...
    if (isNotBlank(idArr)) {
        layer.confirm("确定要批量转移销售线索相关信息吗？", {
            closeBtn: false,
            skin: 'layui-layer-molv'
        }, function () {
            var url = ctx + "/inner/user/selectAllUserSalesLeads?id=" + idArr;
            var title = "批量转移销售线索";
            layer.open({
                type: 2,
                title: title,
...
function renderSalesLeadsName(id, name) {
...
function deleteById(id, name) {
    layer.confirm("确定要删除销售线索【" + name + "】相关信息吗？", {
        closeBtn: false,
        skin: 'layui-layer-molv'
    }, function () {
...
function querySalesLeads(){
    $("#myDataTable").dataTable().fnDestroy();
    var salesLeadsName = $("#txtCustomerName").val();
    $("#myDataTable").dataTable({
        ajax : {
            type: "GET",
            url: ctx + '/inner/salesLeads/listAjax',
            // 传入已封装的参数
            data: function(data){
                data.salesLeadsName = salesLeadsName;
        ...
        "columns": [
            CONSTANT.DATA_TABLES.COLUMN.CHECKBOX,
            {
                ...
                bSortable: false,
                visible: true,
                render: function (data, type, salesLeads) {
                    ...
                    operResult += " <li><a href=\"#\" onclick=\"transfer('" + salesLeads.id + "', '" + salesLeads.customerName + "');\">转移</a></li>";
                    operResult += " <li><a href=\"#\" onclick=\"deleteById('" + salesLeads.id + "', '" + salesLeads.customerName + "');\">删除</a></li>";
                    operResult += " <li><a href=\"#\" onclick=\"editSalesLeads('" + salesLeads.id + "');\">编辑</a></li>";
            ...
            { data: 'customerName',
                render: function(data, type, salesLeads){
                    return renderCustomerName(salesLeads.id, salesLeads.customerName);
                }
            },
            { data: 'customerCompanyName' },
            { data: 'customerDepartmentName'
            },
            /*{ data: 'imgPath' },*/
            { data: 'createTime' }
        ],
...
function initDataTable() {
    $("#myDataTable").dataTable({
        ...
        "columns": [
            ...
            { data:'followUpStatus',
                render:function (data,type,salesLeads) {
                    return getFollowUpStatus(salesLeads.followUpStatus);
                }
            },
            { data: 'customerPhoneNum' },
            { data: 'customerMobileNum' },
            { data: 'createTime' }
        ],
        "bProcessing": true, //DataTables载入数据时，是否显示‘进度’提示
        "bPaginate": true,
...
function getFollowUpStatus(salesLeadsState){
    var stateStr = "";
    if(salesLeadsState == 1){
        stateStr = "未处理";
    }else if(salesLeadsState == 2){
        stateStr = "已联系";
    }else if(salesLeadsState == 3){
        stateStr = "关闭";
    }

    return stateStr;
}
function getSalesLeadsBusinessType(businessType){
    var businessTypeStr = "";
    if(businessType == 0){
