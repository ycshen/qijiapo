function addAttn(){
	var url = ctx + "/inner/attn/forwardEdit";
	layer.open({
		type: 2,
		title: '新增联系人',
		shadeClose: true,
...
function editAttn(id){
	var url = ctx + "/inner/attn/forwardEdit?id=" + id;
	layer.open({
		type: 2,
		title: '编辑联系人',
		shadeClose: true,
...
function viewDetail(id, attnName){
	var url = ctx + "/inner/attn/detail?id=" + id;
	var title = "联系人【" + attnName   +"】";
	layer.open({
		type: 2,
		title: title,
		shadeClose: true,
...
function batchDelete(){
	...
	if(isNotBlank(deleteIdArr)){
		layer.confirm("确定要批量删除联系人相关信息吗？",{closeBtn: false,
	  		skin: 'layui-layer-molv'
		  }, function(){
			  var url = ctx + "/inner/attn/batchDeleteById?ids=" + deleteIdArr;
			$.ajax({
				type: "get",
				url: url,
				success: function(result){
					if(result == 2){
						layer.alert("批量删除联系人成功",{closeBtn: false,
					  		skin: 'layui-layer-molv'
						  }, function(){
							  refreshTable();
							  layer.closeAll();
						...
						layer.alert("批量删除联系人失败",{closeBtn: false,
					  		skin: 'layui-layer-molv'
						  });
					}
				}
		...
		layer.alert("请选择需要批量删除的联系人",{closeBtn: false,
	  		skin: 'layui-layer-molv'
		  });
	}
}
...
function batchTransfer(){
	...
	if(isNotBlank(idArr)){
		layer.confirm("确定要批量转移联系人相关信息吗？",{closeBtn: false,
	  		skin: 'layui-layer-molv'
		  }, function(){
		...
		layer.alert("请选择需要批量转移的联系人",{closeBtn: false,
	  		skin: 'layui-layer-molv'
		  });
	}
}

function renderAttnName(id, name){
...
function queryAttn(){
	...
	$("#myDataTable").dataTable( {
	       ajax : {  
	            type: "GET",  
	            url: ctx + '/inner/attn/listAjax',  
	            // 传入已封装的参数  
	            data: function(data){ 
	            	data.provinceId = provinceId;
	            	data.cityId = cityId;
	            	data.areaId = areaId;
	            	data.attnName = attnName;
	       	...
	       	"columns": [
	             ...
	             {  
	                 ...
	                 bSortable : false,  
	                 visible : true,  
	                 render : function(data, type, attn) {  
	                	 ...
	                	 operResult += 	" <li><a href=\"#\" onclick=\"transfer('" + attn.id + "', '" + attn.attnName + "');\">转移</a></li>";
	                	 operResult += 	" <li><a href=\"#\" onclick=\"deleteById('" + attn.id + "', '" + attn.attnName + "');\">删除</a></li>";
	                	 operResult += 	" <li><a href=\"#\" onclick=\"editAttn('" + attn.id + "');\">编辑</a></li>";
	             ...
	             { data: 'attnName',
	            	render: function(data, type, attn){
	            		return renderAttnName(attn.id, attn.attnName);
	            	}
	             },
	             { data: 'userName' },
	             { data: 'departmentName' },
