...
<input type="hidden" id="hidProductId" name="id" value="${product.id}"/>
<input type="hidden" id="hidBeyondDeptId" name="departmentId" value="${product.departmentId}"/>
<input type="hidden" id="hidBeyondDeptName" name="departmentName" value="${product.departmentName}"/>
...
<div class="container content_div">
  ...
  <div class="layui-form-item my-layui-form-item my-top">
    ...
    <div class="layui-input-block">
      <input type="text" name="productName" lay-verify="productName" autocomplete="off" placeholder="请输入产品名称" class="layui-input" maxlength="10" value="${product.productName}">
    </div>
  </div>
 ...
 <div class="layui-form-item my-layui-form-item my-top">
  ...
  <div id="divViewer">
    ...
    <div class="layui-input-block">
      <a href="#" onclick="addMoreInfo();" style="color:#009688">
      	...
      	<c:choose>
      		<c:when test="${product != null && product.id != '' }">
      		编辑更多信息
      		</c:when>
      		<c:otherwise>
      		新增更多信息
  ...
  <div id="divOther" style="display: none;">
   ...
   <div class="layui-form-item my-layui-form-item my-top">
    ...
    <div class="layui-input-block">
      <input type="text" name="facsimile" autocomplete="off"  value="${product.facsimile}"  placeholder="请输入传真" class="layui-input" maxlength="10">
    </div>
  </div>
  ...
  <div class="layui-form-item my-layui-form-item layui-form-text">
    ...
    <div class="layui-input-block">
      <textarea placeholder="请输入备注" class="layui-textarea" name="remark"> ${product.remark} </textarea>
    </div>
  </div>
</div>
</div>	
...
layui.use(['form', 'layedit', 'laydate'], function(){
  ...
  form.on('submit(mySubmit)', function(data){
	var url = ctx + "/inner/product/saveOrUpdate"
	

	var product =  $('#myForm').serialize();
    $.ajax({
        cache: true,
        type: "POST",
        url: url,
