...
<jsp:include page="../share/qjp_admin_menu.jsp">
	<jsp:param name="nav" value="role_list" />
</jsp:include>
      ...
      <div class="row">
        ...
        <div class="box-body">
			  ...
			  <div class="form-group">
			    <input type="text" class="form-control" id="txtRoleName" placeholder="角色名称">
			  </div>
			...
			<h5>说明：</h5>
              ...
              <table id="syslist" class="table table-bordered table-hover">
                ...
                <tbody>
                	<c:if test="${roleQuery.items != null &&  roleQuery.items.size() > 0}">
                		<c:forEach items="${roleQuery.items}" var="role" varStatus="status">
                			<tr onclick="showRoleDetail('${role.id}', '${role.roleName}')">
                				<td>${status.index + 1 }</td>
		                		<td>${role.roleName }</td>
		                		<td id="roleCount${role.id}">${role.count }</td>
		                		<td id="roleUser${role.id}">${role.userList }</td>
	                		</tr>
                		</c:forEach>
                	</c:if>
                </tbody>
            ...
            <div class="box-header" style="text-align:center">
            	<%@include file="role_list_page.jsp" %>
					<input type="hidden" value="${roleQuery.count}" id="count"/>
